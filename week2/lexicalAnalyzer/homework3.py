...
states = (
    ...
    ('eolcomment', 'exclusive'),
)

# -------------------
# comment definitions
#
def t_multilinecomment(token):
    r'/\*'
    token.lexer.begin('multilinecomment') # switch mode

def t_multilinecomment_end(token):
    r'\*/'
    token.lexer.begin('INITIAL') # switch back to default mode

def t_multilinecomment_error(token):
    token.lexer.skip(1) # pass

def t_eolcomment(token):
    r'//'
    token.lexer.begin('eolcomment') # switch mode

def t_eolcomment_end(token):
    r'\n'
    token.lexer.begin('INITIAL') # switch back to default mode

def t_eolcomment_error(token):
    token.lexer.skip(1) # pass
