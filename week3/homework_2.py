...
def cfgempty(grammar,symbol,visited):
    # base case
    if not any([t==rule[0] for t in symbol for rule in grammar])
        # it's a terminal:
        return symbol
    ...
    for pos in range(len(symbol)):
        for rule in grammar:
            if symbol[pos] == rule[0]:
                s1 = symbol[0:pos] + rule[1] + symbol[pos+1:]
                if not s1 in visited:
                    # TODO:
                    # add to visited
                    # result = result | cfgempty(grammar, s1, visited)


# how to hceck if a symbol is a terminal?
