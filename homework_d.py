...
def nfsmaccepts(current, edges, accepting, visited): 
    ...
    for edge in edges:
        if edge[0]==current:
            # get non-visited destinations
            destinations = set(edges[edge]) - set(visited)
            for d in [dest for dest in edges[edge] if dest not in visited]:
                result = nfsmaccepts(d, edges, accepting, visited)
                if not result == None:
                    # we've found a path that matches - yay!
