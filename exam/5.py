...
def reverse(edges,accepting,start): 
    ...
    dests = set([j for i in edges.values() for j in i])
    
    for d in dests:
        # get incoming
        incoming = [(s, a) for ((s, a), ds) in edges.items() if d in ds]
        
        # for each incoming action
        for a in set([a for (s,a) in incoming]):
            # get states
            states = [s1 for (s1, a1) in incoming if a1 == a]
            # add rule (d, a) -> [s1, s2, ...]
            nEdges[(d, a)] = states 
            
    return (nEdges, start, accepting)
