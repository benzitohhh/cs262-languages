...
def p_exp_tuple(p):
    'exp : LPAREN tupleparams RPAREN'
    p[0] = ("tuple", p[2])

def p_exp_tupleparams_exp_exp(p):
    'tupleparams : exp COMMA exp'
    p[0] = [p[1], p[3]]

def p_exp_tupleparams_exp_tupleparams(p):
    'tupleparams : exp COMMA tupleparams'
    p[0] = [p[1]] + p[3]

def p_exp_list(p):
    'exp : LBRACKET listparams RBRACKET'
    p[0] = ("list", p[2])

def p_exp_explist_exp(p):
    'listparams : exp'
    p[0] = [p[1]]

def p_exp_explist_exp_explist(p):
    'listparams : exp COMMA listparams'
    p[0] = [p[1]] + p[3]
