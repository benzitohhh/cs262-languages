    ...
    if stmttype == "call": # ("call", "sqrt", [("number","2")])
        ...
        if fvalue[0] == "function":
            ...
            if len(fparams) <> len(args):
                ...
                # IMPORTANT!!!!
                # new env's parent pointer should be fenv,
                # NOT environment
                newenv = (fenv, {})
                # evaluate each argument in turn - but from CURRENT env (not NEW)
                for i in range(len(args)):
                    argval = eval_exp(args[i], environment)
                    (newenv[1])[fparams[i]] = argval # add to NEWENV's dict i.e. dict[x] = 2
                
                try:
                    # IMPORTANT!!!
                    # need to call function in NEWENV
...
sqr = ("function",("x"),(("return",("binop",("identifier","x"),"*",("identifier","x"))),),{})

environment = (None,{"sqr":sqr})

print eval_stmt(("call","sqr",[("number","2")]),environment)    
